...
- Uses a **short-lived memory cache** (powered by `cachebox.TTLCache`) for worker data to avoid redundant Redis hits.
...
- **Token Hashing**: Token hashes are cached in memory (using `cachebox.TTLCache`) to minimize CPU usage during authentication.
