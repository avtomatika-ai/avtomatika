...
- **Высокопроизводительное хранилище Redis**: Индексация через **ZSET** для атомарного управления воркерами и сериализация **Msgpack**.
- **Безопасность Zero Trust**: Обязательная аутентификация воркеров и **Защита от повторов** (таймстемп-окно 60с).
- **Подключаемое Blob-хранилище**: Полная поддержка S3 через интерфейс `BlobProvider` (на базе `obstore`).
- **Иерархическая логика**: Поддержка дочерних блюпринтов (Ghosts) и параллельного выполнения задач с агрегацией результатов.
- **Observability**: Распределенная трассировка с **OpenTelemetry** и встроенные метрики.
