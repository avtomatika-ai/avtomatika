...
*   **Как это работает**: При запуске движок сканирует директорию на наличие всех `.py` файлов, импортирует их и автоматически регистрирует все найденные экземпляры `StateMachineBlueprint`.
