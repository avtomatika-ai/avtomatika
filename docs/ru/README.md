...
Оркестратор включает несколько механизмов корпоративного уровня для обеспечения целостности системы:

*   **Экспоненциальная задержка (Exponential Backoff):** Основные циклы (`JobExecutor`, `Watcher`) автоматически применяют стратегию экспоненциальной задержки при сбоях инфраструктуры (например, сбоях Redis), что позволяет системе самовосстанавливаться, не перегружая ресурсы.
*   **Защита от захвата задач (Job Hijacking):** Строгий контроль владения гарантирует, что только воркер, которому назначена задача, может отправить её результат. Несанкционированные попытки блокируются и фиксируются как инциденты безопасности.
*   **Защита от циклов:** Настройка `MAX_TRANSITIONS_PER_JOB` (по умолчанию 100) автоматически завершает блупринты, вошедшие в бесконечные логические петли.
*   **Защита от устаревших результатов:** Система автоматически игнорирует результаты задач, которые уже были перезапущены по таймауту, обеспечивая целостность данных.
