...
### Каталог навыков (Marketplace)

-   **Эндпоинт:** `GET /api/v1/workers/catalog`
-   **Описание:** Возвращает агрегированный каталог всех уникальных навыков, доступных в сети, с их полными контрактами (схемы, статусы, цены).
-   **Ответ (`200 OK`):** JSON-объект, где ключи - имена навыков.

---

## 3. Внутренние эндпоинты для Воркеров (`/_worker`)
...
### Отправка произвольного события (Bottom-Up)

-   **Эндпоинт:** `POST /_worker/events`
-   **Описание:** Позволяет воркеру отправить любой сигнал (событие) Оркестратору.
-   **Тело запроса:** `WorkerEventPayload` (включая `origin_worker_id` и `bubbling_chain`).
-   **Ответ (`200 OK`):** `{"status": "event_accepted"}`
