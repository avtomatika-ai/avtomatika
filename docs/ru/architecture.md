...
-   **Модели:** Структуры данных (`WorkerRegistration`, `TaskResult`), определенные как `NamedTuple`.
-   **Схемы и Контракты:** Модуль `rxon.schema` предоставляет механизмы автоматического извлечения JSON-схем из типов Python и их валидации.
-   **Универсальные события:** Единая модель `WorkerEventPayload` для всех типов сигналов (прогресс, алерты, логи).
-   **Безопасность:** Цепочка идентичности (Zero Trust) для отслеживания пути событий через иерархию.
...
-   **`WorkerService`**: Управляет жизненным циклом воркеров, регистрацией, распределением задач, обработкой результатов, выдачей STS токенов и **валидацией контрактных событий**.
-   **Глобальный реестр контрактов**: Оркестратор сохраняет схемы всех Блюпринтов в Redis, обеспечивая синхронизацию контрактов во всем кластере.
...
- **Регистрация "Блупринтов":** Поддерживает как ручную регистрацию `Blueprint`, так и автоматическую загрузку из директории, указанной в `BLUEPRINTS_DIR`.
...
- **Хендлеры:** Функции, привязанные к определенным состояниям с помощью декоратора `@blueprint.handler`. Они получают `JobContext` и `ActionFactory` для выполнения действий.
- **Явное определение состояний:** Декоратор `@blueprint.handler` принимает два булевых флага:
...
- **Управление состоянием:** После выполнения хендлера он обрабатывает действия, запрошенные через `ActionFactory`. Это может быть простой переход в новое состояние (`go_to`) или более сложная логика, например, отправка задачи воркеру (`dispatch_task`).
  ...
  **Реактивные переходы (Event-Driven Transitions)**

  Система поддерживает смену состояния Блюпринта на основе промежуточных событий от воркера, не дожидаясь финального результата задачи. Это реализуется через параметр `event_transitions` в методе `dispatch_task`. Если воркер присылает событие, указанное в этом словаре, Оркестратор немедленно переводит задачу в новое состояние, а финальный результат исходной задачи будет проигнорирован.
...
- **Отказоустойчивость:** Реализует логику повторных попыток (retry) и перемещение невыполнимых задач в карантин.

  **Механизмы надежности и защиты**

  Система внедряет несколько продвинутых механизмов для обеспечения стабильности и безопасности в агрессивных или нестабильных средах:

  - **Защита от «захвата» задач (Job Hijacking):** Оркестратор строго контролирует владение задачами. Когда воркер отправляет результат, система проверяет, что именно этот воркер был назначен на задачу. Попытки подмены блокируются и логируются как критические события безопасности.
  - **Защита от устаревших результатов:** Если задача была переназначена (например, по таймауту), система проигнорирует «запоздалые» результаты от предыдущего воркера, предотвращая порчу состояния процесса.
  - **Предотвращение бесконечных циклов:** Для защиты от ошибок проектирования блупринтов лимит `MAX_TRANSITIONS_PER_JOB` (по умолчанию 100) гарантирует принудительное завершение задач, попавших в логические петли.
  - **Экспоненциальная задержка (Exponential Backoff):** Основные фоновые циклы (`JobExecutor`, `Watcher`) используют стратегию экспоненциальной задержки при возникновении временных инфраструктурных сбоев (например, проблем с подключением к Redis), обеспечивая самовосстановление системы.
  ...
  **Механизмы протухания (Timeouts):**
  Оркестратор поддерживает два типа таймаутов для задач:
  - **`dispatch_timeout`** (Таймаут в очереди): Максимальное время, которое задача может ожидать свободного воркера. Если ни один воркер не забрал задачу за это время, она отменяется.
  - **`result_timeout`** (Дедлайн результата): Абсолютный дедлайн для получения результата с момента диспатча. Если воркер взял задачу, но не успел прислать ответ до этого момента, результат будет проигнорирован, а задача помечена как проваленная.
  ...
  **Параллельное выполнение задач**
  ...
  2.  **Агрегация результатов:**
      - Хендлер, предназначенный для сбора результатов, помечается специальным декоратором `@blueprint.aggregator`.
...
**Ключевые методы:**
- `actions.go_to(state: str)`: Просто переводит конечный автомат в следующее состояние. Выполнение продолжится немедленно.
...
- **Deep Schema Matching (Глубокая проверка контрактов):** Оркестратор проверяет параметры задачи против `input_schema` воркера с помощью утилиты `validate_data`. Задача назначается только тому, кто гарантированно понимает формат данных.
- **Status Matching:** Проверка совместимости логических выходов (статусов) воркера с переходами, ожидаемыми Блюпринтом. Это предотвращает "зависание" пайплайна.
...
- **Стратегии:** К отфильтрованному пулу воркеров применяет одну из стратегий выбора: `default`, `round_robin` (использует кластерный счетчик в Redis), `least_connections`, `cheapest`, `best_value` (с учетом репутации).
- **Постановка в очередь:** Помещает задачу в персональную приоритетную очередь выбранного воркера в `Storage`.

### 4. Диспетчер
**Расположение:** `src/avtomatika/dispatcher.py`

Отвечает за назначение задач наиболее подходящему воркеру.
- **Поиск воркеров O(1):** Использует пересечение множеств Redis (`SINTER`) для мгновенного нахождения свободных воркеров, способных выполнить конкретный тип задачи. Это заменяет сканирование O(N) и обеспечивает постоянную производительность даже при тысячах воркеров.
- **Фильтрация:** 
    1. **Статус и тип задачи:** Обрабатываются мгновенно через индексацию Redis.
    2. **Требования к ресурсам:** Фильтрует полученных кандидатов по характеристикам оборудования (модель GPU, объем VRAM, установленные ML-модели).
- **Стратегии:** Применяет один из следующих алгоритмов выбора к финальному пулу кандидатов:
    - `default`: Отдает приоритет «теплым» воркерам (с нужными моделями в памяти), затем выбирает воркера с самой высокой репутацией среди самых дешевых.
    - `overflow`: «Перелив на более дорогих». Пытается назначить задачу самому дешевому воркеру, чья длина очереди ниже `DISPATCHER_SOFT_LIMIT`. Если все дешевые воркеры заняты, нагрузка переливается на более дорогих.
    - `round_robin`: Распределяет нагрузку последовательно, используя счетчик в Redis.
    - `least_connections`: Выбирает воркера с наименьшим количеством активных задач.
    - `cheapest`: Выбирает самого дешевого воркера для конкретного навыка.
    - `best_value`: Выбирает воркера с лучшим соотношением цена/качество, используя его репутацию.
- **Work Stealing:** Для предотвращения блокировки очереди (Head-of-Line Blocking) слой хранилища реализует механизм атомарной кражи задач («Work Stealing»). Если личная очередь воркера пуста, Оркестратор (через Lua-скрипт в Redis) может найти самую длинную очередь среди других воркеров с таким же навыком и перенести задачу оттуда свободному исполнителю.
- **Постановка в очередь:** Помещает задачу в приоритетную очередь воркера в Хранилище.

### 4.1. Взаимодействие с воркерами (Pull Model)

Система использует **Pull-модель**, при которой воркеры сами инициируют соединение с оркестратором для получения задач. Это позволяет воркерам находиться за NAT или файерволами.
...
Это компонент, отвечающий за анализ производительности воркеров и управление доверием в сети.
- **Мгновенная саморегуляция:** В отличие от фонового пересчета, система теперь применяет немедленные штрафы за нарушение схем данных (`REPUTATION_PENALTY_CONTRACT_VIOLATION`) или критические сбои, а также награды (`REPUTATION_REWARD_SUCCESS`) за успешное выполнение. Это создает динамическую экономику доверия в реальном времени.
- **Порог допуска:** Настраиваемый параметр `REPUTATION_MIN_THRESHOLD` позволяет автоматически отключать нестабильные или некачественные узлы от сети.
- **Фоновая калибровка:** Периодически (раз в час) запрашивает из `HistoryStorage` историю задач за последние 30 дней для долгосрочной корректировки оценки на основе статистического анализа.
