...
| `dispatch_timeout` | Integer | Нет | Таймаут ожидания воркера (сек). Задача провалится, если не будет взята из очереди. |
| `result_timeout` | Integer | Нет | Дедлайн результата (сек). Абсолютное время с момента создания до получения ответа. |
...
| `BLUEPRINTS_DIR` | Путь к директории с Python-файлами, содержащими блупринты (`Blueprint`), для их автоматической загрузки. | `""` |
...
| `DISPATCHER_SOFT_LIMIT` | **Стратегия перелива:** Максимальное число задач в очереди воркера до переключения на более дорогих кандидатов. | `3` |
| `WORK_STEALING_ENABLED` | **Кража задач:** Если `true`, свободные воркеры могут атомарно «красть» задачи у перегруженных коллег. | `true` |
...
| `MAX_TRANSITIONS_PER_JOB` | **Защита от циклов:** Максимальное количество переходов между состояниями для одного задания, после которого оно принудительно завершается. | `100` |
...
| `S3_AUTO_CLEANUP` | Если `true`, автоматически удаляет файлы в S3 и локальные временные артефакты после завершения или сбоя задания. Установите `false`, если хотите управлять очисткой вручную или через S3 Lifecycle Policies. | `true` |
