...
*   **`rxon_message:{type}`**: Создается в `engine.py` при получении любого сообщения от воркера (`poll`, `result`, `heartbeat`).
    *   *Атрибуты*: `worker.id_hint`, `message.type`, `auth.worker_id`.
*   **`JobExecutor:{blueprint}:{state}`**: Создается при выполнении шага логики в оркестраторе.
    *   *Атрибуты*: `job.id`, `job.blueprint`, `job.client_token`, `job.retry_count`.
    *   *События*: Ошибки записываются через `span.record_exception(e)` с полным стек-трейсом.
...
*Примечание: Если `OTEL_EXPORTER_OTLP_ENDPOINT` не задан, данные будут выводиться в консоль (ConsoleExporter), что удобно для проверки корректности работы без развертывания Jaeger.*
